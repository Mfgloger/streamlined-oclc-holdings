...
from db_access import session_scope, insert_or_ignore
from utils import save2csv, start_from_scratch
...
def select_for_sierra_list_creation(n=int) -> None:
    """
    ...
    out_fh = f"./files/enhanced/BPL/batch-{timestamp:%y%m%d}-sierra-nos.csv"  # for archival purposes
    temp_fh = os.path.join(
        os.getenv("USERPROFILE"),
        f"documents/batch-{timestamp:%y%m%d}-sierra-nos.csv",
    )
    out_fh2 = f""
...
def parse_sierra_bib(src_fh: str = None) -> None:
    if src_fh is None:
        timestamp = datetime.now()
        src_fh = os.path.join(
            os.getenv("USERPROFILE"), f"documents/batch2enhance-{timestamp:%y%m%d}.out"
        )
